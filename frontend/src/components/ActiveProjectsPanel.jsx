...
export function ActiveProjectsPanel({ projects, totalActive, title = "Active Projects", showPi = true, sampleMeta }) {
  const list = projects || [];
  const countLabel = totalActive != null ? totalActive : list.length;
  const showingHint =
    sampleMeta && sampleMeta.total > sampleMeta.shown
      ? `Showing ${sampleMeta.shown} / ${sampleMeta.total}`
      : null;
