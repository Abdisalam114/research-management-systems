...
async function buildRepositoryAccessFilter(req) {
  const { role } = req.user;
  ...
  if (projectIdQuery) {
    ...
    const myProjects = await Project.find({
      $or: [{ researcherId: req.user.id }, { "teamMembers.userId": req.user.id }],
    }).select("_id");
...
module.exports = {
  buildRepositoryAccessFilter,
  fetchItemsForUser,
  isSeedRepositoryItem,
  itemsToExportRows,
  rowsToCsv,
  rowsToExcelXml,
