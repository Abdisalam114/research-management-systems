...
async function listMyNotifications(req, res) {
  const notifications = await Notification.find(req.tierWhere({ userId: req.user.id })).sort({ createdAt: -1 }).limit(100);
  res.json({ notifications: notifications.map(sanitizeNotification) });
}

async function markRead(req, res) {
  const { id } = req.params;
  const n = await Notification.findOne(req.tierWhere({ _id: id }));
...
async function unreadCount(req, res) {
  const count = await Notification.countDocuments(req.tierWhere({ userId: req.user.id, readAt: null }));
