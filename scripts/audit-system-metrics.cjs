...
async function main() {
  ...
  const checks = {
    ...
    groupsTotal: institutional.preview?.groups?.total === institutional.overview.groups,
    activeProjectsPreview: institutional.preview?.activeProjects?.total === institutional.projectStatus.active,
    hasMetricDefinitions: Boolean(institutional.metricDefinitions),
    hasPreview: Boolean(institutional.preview),
  };
